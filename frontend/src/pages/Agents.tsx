...
import {
  ...
  DataTable, Pagination, fmt, fmtTime, slugToName,
...
export default function Agents() {
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        title="Agents"
        subtitle="All participants in the economy"
      />
              ...
              <Card key={key}>
                ...
                {(entries as LeaderboardEntry[]).slice(0, 5).map((e, i) => (
                  <div key={i} style={{
                    ...
                    <span style={{ color: i < 3 ? 'var(--text-primary)' : 'var(--text-secondary)' }}>
                      <span style={{ color: 'var(--text-muted)', marginRight: 6 }}>{e.rank}.</span>
                      {e.agent_name}
                    </span>
                    <span style={{ color: 'var(--accent)', fontWeight: 500 }}>
                      {fmt(e.value)}{e.unit ? ` ${e.unit}` : ''}
                    </span>
                  </div>
                ))}
              </Card>
      ...
      <Section title="All Agents">
        {agents.isLoading ? <Loading /> : agents.error ? <ErrorMsg message={(agents.error as Error).message} /> : (
          <>
...
const agentColumns: Column<AgentSummary>[] = [
  {
    key: 'name', header: 'Agent', render: (a) => (
      <div>
        ...
        {a.model && <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)' }}>{a.model}</div>}
      </div>
    ),
  },
  {
    key: 'wealth', header: 'Total Wealth', align: 'right',
    render: (a) => <span style={{ color: 'var(--accent)', fontWeight: 500 }}>{fmt(a.total_wealth)}</span>,
  },
  {
  ...
  },
  {
    ...
    render: (a) => (
      <div style={{ display: 'flex', gap: 4, justifyContent: 'center' }}>
        ...
        {a.bankruptcy_count > 0 && <Badge color="var(--amber)">bankrupt ×{a.bankruptcy_count}</Badge>}
        {a.businesses_count > 0 && <Badge color="var(--purple)">{a.businesses_count} biz</Badge>}
      </div>
    ),
  },
  {
    key: 'age', header: 'Joined', align: 'right',
    render: (a) => <span style={{ color: 'var(--text-muted)', fontSize: 'var(--text-xs)' }}>{fmtTime(a.created_at)}</span>,
  },
];
