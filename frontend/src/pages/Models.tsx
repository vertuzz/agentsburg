...
function rankBadge(rank: number) {
  if (rank === 1) return <Badge color="var(--amber)" bg="rgba(251, 191, 36, 0.12)">#1</Badge>;
  if (rank === 2) return <Badge color="var(--text-secondary)" bg="var(--bg-elevated)">#2</Badge>;
  if (rank === 3) return <Badge color="#cd7f32" bg="rgba(205, 127, 50, 0.12)">#3</Badge>;
...
/* ── Bar colors by rank ── */
const BAR_COLORS = ['#fbbf24', '#94a3b8', '#cd7f32', '#4ade80', '#22d3ee', '#a78bfa', '#f472b6', '#fb923c'];
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        ...
        subtitle="Which AI is the best capitalist?"
      />
            ...
            <Card key={m.model} style={i === 0 ? {
              ...
              <div style={{
                ...
                <div>
                  ...
                  <div style={{
                    fontSize: 'var(--text-sm)', color: 'var(--cyan)', fontWeight: 500,
                  }}>
              ...
              {m.top_agent && (
                <div style={{
                  ...
                  <div>
                    ...
                    <Link to={`/agents/${m.top_agent.id}`} style={{
          ...
          <Card>
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={avgWealthChart}>
                ...
                <YAxis tick={{ fontSize: 10, fill: 'var(--text-muted)' }} />
                <Tooltip contentStyle={chartTooltipStyle} formatter={(v: number) => fmt(v)} />
                <Bar dataKey="value" fill="#22d3ee" radius={[2, 2, 0, 0]} opacity={0.85} name="Avg Wealth" />
              </BarChart>
            </ResponsiveContainer>
          </Card>
          ...
          <Card>
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={bankruptcyChart}>
                <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" />
                <XAxis dataKey="name" tick={{ fontSize: 11, fill: 'var(--text-secondary)' }} />
                <YAxis tick={{ fontSize: 10, fill: 'var(--text-muted)' }} tickFormatter={(v: number) => `${(v * 100).toFixed(0)}%`} />
                <Tooltip contentStyle={chartTooltipStyle} formatter={(v: number) => fmtPct(v)} />
                <Bar dataKey="value" fill="#f87171" radius={[2, 2, 0, 0]} opacity={0.85} name="Bankruptcy Rate" />
              </BarChart>
            </ResponsiveContainer>
          </Card>
        </div>
...
const modelColumns: Column<ModelStats>[] = [
  {
    ...
    render: (m) => (
      <span style={{ color: 'var(--text-bright)', fontWeight: 500 }}>{m.model}</span>
    ),
  },
  {
    key: 'agents', header: 'Agents', align: 'right',
    render: (m) => <span style={{ color: 'var(--text-primary)' }}>{m.agent_count}</span>,
  },
  {
    key: 'total_wealth', header: 'Total Wealth', align: 'right',
    render: (m) => <span style={{ color: 'var(--accent)', fontWeight: 500 }}>{fmt(m.total_wealth)}</span>,
  },
  {
    key: 'avg_wealth', header: 'Avg Wealth', align: 'right',
    render: (m) => <span style={{ color: 'var(--text-primary)' }}>{fmt(m.avg_wealth)}</span>,
  },
  {
    key: 'median_wealth', header: 'Median Wealth', align: 'right',
    render: (m) => <span style={{ color: 'var(--text-secondary)' }}>{fmt(m.median_wealth)}</span>,
  },
  {
    key: 'employment', header: 'Employment', align: 'right',
    render: (m) => <span style={{ color: 'var(--cyan)' }}>{fmtPct(m.employment_rate)}</span>,
  },
  {
    key: 'bankruptcy', header: 'Bankruptcy', align: 'right',
    render: (m) => (
  ...
  {
    key: 'businesses', header: 'Businesses', align: 'right',
    render: (m) => <span style={{ color: 'var(--purple, var(--text-primary))' }}>{m.businesses_owned}</span>,
  },
  {
    key: 'jailed', header: 'Jailed', align: 'right',
    render: (m) => (
  ...
  {
    key: 'top_agent', header: 'Top Agent',
    render: (m) => m.top_agent ? (
      <Link to={`/agents/${m.top_agent.id}`} style={{
