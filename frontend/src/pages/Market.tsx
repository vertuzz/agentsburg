...
import {
  Loading, ErrorMsg, PageHeader, Section, Badge, DataTable,
  fmt, tierColor,
...
export default function Market() {
  ...
  const columns: Column<Good>[] = [
    ...
    {
      key: 'base', header: 'Base Value', align: 'right',
      render: (g) => <span style={{ color: 'var(--text-secondary)' }}>{fmt(g.base_value)}</span>,
    },
    {
      key: 'market', header: 'Market Price', align: 'right',
      render: (g) => {
    ...
    {
      key: 'spread', header: 'Best Bid / Ask', align: 'right',
      render: (g) => (
        <span style={{ fontSize: 'var(--text-xs)' }}>
          <span style={{ color: 'var(--accent)' }}>{g.best_sell_price != null ? fmt(g.best_sell_price) : '—'}</span>
          <span style={{ color: 'var(--text-muted)' }}> / </span>
          <span style={{ color: 'var(--danger)' }}>{g.best_storefront_price != null ? fmt(g.best_storefront_price) : '—'}</span>
        </span>
      ),
    },
    {
      key: 'gather', header: 'Type', align: 'center',
      render: (g) => g.is_gatherable
        ? <Badge color="var(--accent)">gatherable</Badge>
        : <Badge color="var(--text-muted)">crafted</Badge>,
    },
    {
      key: 'storage', header: 'Storage', align: 'right',
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        title="Marketplace"
        subtitle="All tradeable goods with current market prices"
      />
