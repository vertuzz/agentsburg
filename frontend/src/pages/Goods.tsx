...
import { fmt, slugToName, tierColor } from "../components/formatters";
import type { Good } from "../types";

// Known recipes (static — could be loaded from backend in the future)
const RECIPES: Record<
  string,
  { inputs: { slug: string; qty: number }[]; output: number; business: string }
> = {
  ...
  clothing: {
    ...
    output: 2,
    business: "textile_shop",
  },
  pottery: {
    inputs: [
      { slug: "clay", qty: 2 },
      { slug: "herbs_dried", qty: 1 },
    ],
    output: 2,
    business: "kiln",
  },
  ...
  jewelry: {
    inputs: [
      { slug: "copper_ingots", qty: 1 },
      { slug: "iron_ingots", qty: 1 },
      { slug: "glass", qty: 1 },
    ],
    output: 1,
    business: "jeweler",
      ...
      <Section title="Supply Chain">
        <Card>
          ...
          >
            ...
            <div style={{ textAlign: "center" }}>
              <div style={{ fontSize: "var(--text-sm)", color: "var(--cyan)", fontWeight: 500 }}>
                Tier 2
              </div>
            ...
            <span style={{ fontSize: "1.5rem", color: "var(--text-muted)" }}>→</span>
            <div style={{ textAlign: "center" }}>
              <div style={{ fontSize: "var(--text-sm)", color: "var(--amber)", fontWeight: 500 }}>
                Tier 3
              </div>
    ...
    <Card>
      ...
      >
        <div>
          <Link
            to={`/market/${g.slug}`}
            style={{ fontSize: "var(--text-base)", fontWeight: 600, color: tierColor(g.tier) }}
          >
            {g.name}
          </Link>
        ...
        <div style={{ textAlign: "right" }}>
          {marketPrice != null ? (
            <div style={{ fontSize: "var(--text-lg)", color: "var(--accent)", fontWeight: 600 }}>
              {fmt(marketPrice)}
            </div>
          ) : (
