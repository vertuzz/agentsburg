...
async def process_loan_payments(
    ...
    result = await db.execute(
        select(Loan).where(Loan.status == "active", Loan.next_payment_at <= now)
    )
    due_loans = list(result.scalars().all())

    if not due_loans:
        return {"type": "loan_payments", "processed": 0, "paid": 0,
                "defaulted": 0, "total_collected": 0.0}
    ...
    for loan in due_loans:
        ...
        if agent_balance >= installment:
            ...
            paid_count += 1
            db.add(Transaction(
                type="loan_payment", from_agent_id=agent.id, to_agent_id=None,
                amount=installment, metadata_json={
                    "loan_id": str(loan.id),
            ...
            db.add(Transaction(
                type="loan_payment", from_agent_id=agent.id, to_agent_id=None,
                amount=Decimal("0"), metadata_json={
                    "loan_id": str(loan.id), "status": "defaulted",
    ...
    return {
        "type": "loan_payments", "processed": len(due_loans),
        "paid": paid_count, "defaulted": defaulted_count,
        "total_collected": float(total_collected),
    }


async def default_agent_loans(
    db: AsyncSession, agent: Agent, clock: "Clock",
) -> dict:
    """Default all active loans for a bankrupt agent, writing off remaining balances."""
    now = clock.now()

    result = await db.execute(
        select(Loan).where(Loan.agent_id == agent.id, Loan.status == "active")
    )
    ...
    for loan in active_loans:
        ...
        db.add(Transaction(
            type="loan_payment", from_agent_id=agent.id, to_agent_id=None,
            amount=Decimal("0"), metadata_json={
                "loan_id": str(loan.id), "status": "defaulted_bankruptcy",
...
async def close_bank_account_for_bankruptcy(
    ...
    for loan in active_loans:
        ...
        if remaining_deposits >= loan_remaining:
            ...
            db.add(Transaction(
                type="loan_payment", from_agent_id=agent.id, to_agent_id=None,
                amount=loan_remaining, metadata_json={
                    "loan_id": str(loan.id),
            ...
            if remaining_deposits > 0:
                ...
                db.add(Transaction(
                    type="loan_payment", from_agent_id=agent.id, to_agent_id=None,
                    amount=remaining_deposits, metadata_json={
                        "loan_id": str(loan.id),
    ...
    if remaining_deposits > 0:
        ...
        db.add(Transaction(
            type="withdrawal", from_agent_id=None, to_agent_id=agent.id,
            amount=remaining_deposits, metadata_json={
