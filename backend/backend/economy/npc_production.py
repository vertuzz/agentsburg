...
async def _auto_restock_inputs(
    db: AsyncSession, biz: Business, recipe: Recipe,
    effective_quantity: int, central_bank: CentralBank, settings: "Settings",
    ...
    for inp in (recipe.inputs_json or []):
        ...
        inv_r = await db.execute(select(InventoryItem).where(
            InventoryItem.owner_type == "business",
            InventoryItem.owner_id == biz.id, InventoryItem.good_slug == inp_slug,
        ))
...
async def run_npc_production(
    db: AsyncSession, npc_businesses: list[Business],
    agents_map: dict[str, Agent], recipes_by_slug: dict[str, Recipe],
    npc_config_by_name: dict[str, dict], central_bank: CentralBank | None,
    npc_efficiency: float, npc_wage_mult: float, settings: "Settings",
) -> list[dict[str, Any]]:
        ...
        for prod_cfg in npc_config_by_name.get(biz.name, {}).get("produces", []):
            ...
            if recipe is not None:
                ...
                for inp in (recipe.inputs_json or []):
                    inp_slug = inp.get("good_slug") or inp.get("good")
                    inp_qty_needed = max(1, int(int(inp.get("quantity", 1)) * input_scale))
                    inv_result = await db.execute(select(InventoryItem).where(
                        InventoryItem.owner_type == "business",
                    ...
                    if inv_item is None or inv_item.quantity < inp_qty_needed:
                        ...
                        logger.debug(
                            ...
                            biz.name, inp_slug, prod_cfg.get("recipe"),
            ...
            if not inputs_satisfied:
                if recipe is not None and central_bank is not None:
                    inputs_satisfied, inputs_used = await _auto_restock_inputs(
                        db, biz, recipe, effective_quantity, central_bank, settings,
                    )
                if not inputs_satisfied:
                    continue
            ...
            out_inv_result = await db.execute(select(InventoryItem).where(
                InventoryItem.owner_type == "business",
                InventoryItem.owner_id == biz.id,
            ...
            if units_to_add > 0:
                if out_inv is None:
                    out_inv = InventoryItem(
                        owner_type="business", owner_id=biz.id,
                        good_slug=good_slug, quantity=units_to_add,
                    )
                    db.add(out_inv)
                else:
...
async def spawn_demand_gap_businesses(
    ...
    central_bank: CentralBank | None, now,
) -> list[dict]:
    ...
    for demand_cfg in demand_entries:
        ...
        supply_result = await db.execute(
            select(func.sum(InventoryItem.quantity)).where(
                InventoryItem.owner_type == "business",
                InventoryItem.good_slug == good_slug, InventoryItem.quantity > 0,
            ))
        sell_order_result = await db.execute(
            select(func.sum(MarketOrder.quantity_total - MarketOrder.quantity_filled)).where(
                MarketOrder.good_slug == good_slug, MarketOrder.side == "sell",
        ...
        npc_agent = Agent(
            name=npc_name[:64], balance=float(initial_balance),
        ...
        new_biz = Business(
            owner_id=npc_agent.id, name=f"NPC {good_slug.replace('_', ' ').title()} Co.",
            type_slug=recipe.bonus_business_type or "workshop",
            zone_id=spawn_zone.id, storage_capacity=500, is_npc=True,
        ...
        db.add(StorefrontPrice(
            business_id=new_biz.id, good_slug=good_slug, price=round(ref_price * 1.5, 2),
        ))
        db.add(InventoryItem(
            owner_type="business", owner_id=new_biz.id, good_slug=good_slug,
            quantity=min(100, max(10, int(base_demand * 2))),
        ))
        default_wage = float(getattr(settings.economy, "default_wage_per_work_call", 30))
        db.add(JobPosting(
            business_id=new_biz.id, product_slug=good_slug,
            title=f"{good_slug.replace('_', ' ').title()} Worker",
            wage_per_work=default_wage, max_workers=3, is_active=True,
        ))

        central_bank.reserves = Decimal(str(central_bank.reserves)) - initial_balance
        db.add(Transaction(
            type="loan_disbursement", from_agent_id=None,
            to_agent_id=npc_agent.id, amount=float(initial_balance),
            metadata_json={
                "reason": "npc_business_spawn", "good_slug": good_slug,
                "business_name": new_biz.name, "supply_ticks": ticks_of_supply,
        ...
        spawned.append({
            "business": new_biz.name, "good": good_slug,
            "zone": spawn_zone_slug, "supply_ticks_before": ticks_of_supply,
        })
        logger.info(
